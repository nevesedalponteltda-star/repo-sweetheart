      ...
      <header className="no-print bg-white/80 backdrop-blur-md border-b border-gray-100 sticky top-0 z-50">
        <div className="container mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-10 text-xs font-black uppercase tracking-widest text-gray-400">
            <Link to="/" className={`transition-all ${isActive('/') ? 'text-blue-600 border-b-2 border-blue-600 pb-1' : 'hover:text-gray-900'}`}>
              Minhas Faturas
            </Link>
            <Link to="/clients" className={`transition-all ${isActive('/clients') ? 'text-blue-600 border-b-2 border-blue-600 pb-1' : 'hover:text-gray-900'}`}>
              Clientes
            </Link>
            <Link to="/settings" className={`transition-all ${isActive('/settings') ? 'text-blue-600 border-b-2 border-blue-600 pb-1' : 'hover:text-gray-900'}`}>
              Configurações
            </Link>
            ...
            <div className="relative group">
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-100 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                ...
                <button onClick={handleLogout} className="w-full px-4 py-2 text-left text-sm text-red-600 hover:bg-red-50 font-medium">
                  Sair
                </button>
              </div>
            </div>
